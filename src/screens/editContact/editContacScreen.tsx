...
//   },
// });
//Hasta aqui funciona pero con el id del telefono
...
export const EditScreen: React.FC<any> = ({route}) => {
  const {contact} = route.params;
  console.log(`FROM EDIT: ${JSON.stringify(contact)}`)
  console.log(`CHECKID: ${contact.contactId}`)
  const navigation = useNavigation();
  const [name, setName] = useState(contact.name || '');
  const [telephone, setTelephone] = useState(contact.telephone || '');
  const [email, setEmail] = useState(contact.email || '');
  ...
  console.log(`before Save: ${contact.contactId}`);

  const saveContact = async () => {
    try {
      // Actualizar el contacto usando el ID basado en el número de teléfono original
      ...
      console.log(`FROM SAVE: ${contactId}`)
