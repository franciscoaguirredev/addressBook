...
//   console.log(`desde profile: ${contact}`)
...
//     try {
//       await AsyncStorage.removeItem(`contact_${contact.contactId}`);
...
//   },
// });
// hasta aqui funciona pero con el uuid pero sin eliminar aunque si editar
...
//   console.log(`profile: ${route.contactId}`)
...
//   useEffect(() => {
//     const loadContactData = async () => {
//       const updatedContact = await AsyncStorage.getItem(`contact_${contactId}`);
//       if (updatedContact) {
//         const parsedContact = JSON.parse(updatedContact);
//         setContactData(parsedContact);
...
//   const handleDelete = async () => {
//     try {
//       await AsyncStorage.removeItem(`contact_${contactId}`);
//       Alert.alert('Contact deleted', 'The contact has been successfully deleted.');
//       navigation.goBack();
//     } catch (error) {
//       Alert.alert('Error', 'The contact could not be deleted.');
...
//           style={styles.iconButton}
//           onPress={() =>
//             navigation.navigate('EditContactScreen', {contactId})
//           }>
//           <Icon name="edit" size={24} color="black" />
//           <Text style={styles.text}>Edit</Text>
//         </TouchableOpacity>
...
export const ProfileScreen: React.FC<any> = ({route}) => {
  const {contact} = route.params;
  // console.log(`prueba: ${JSON.stringify(route, null, 2)}`)
  // console.log(`desde profile: ${contact.id}`)
  // console.log(JSON.stringify(contact, null, 2))
  const navigation = useNavigation<any>();
  const [contactData, setContactData] = useState(contact);
  const [location, setLocation] = useState<{ latitude: number; longitude: number } | null>(null);
  ...
  useEffect(() => {
    const loadContactData = async () => {
      console.log(`FROM PROFILE contact_${contact.id}`)
  ...
  const handleDelete = async () => {
    try {
      console.log(`PARA ELIMINAR contact_${contact.id}`)
